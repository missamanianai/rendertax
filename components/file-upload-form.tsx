...
export default function FileUploadForm() {
        ...
        try {
          ...
          setFiles((prev) => [
            ...prev,
            {
              ...
              fileUrl: result.url,
              fileSize: file.size,
              transcriptType: result.transcriptType || "unknown",
  ...
  const canContinue = files.length > 0 && !hasErrors && !uploading

  const handleContinue = () => {
    // Store file IDs in session or pass to next step
    const fileIds = files.map((file) => file.id)
    // Navigate to client information page
    router.push(`/client-info?files=${fileIds.join(",")}`)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        } ${uploading || files.length >= 6 ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        <input {...getInputProps()} />
        <div className="flex flex-col items-center justify-center gap-2">
          ...
          <Button variant="secondary" className="mt-2" disabled={uploading || files.length >= 6}>
            Browse Files
          </Button>
        </div>
      </div>
